...
- **BlockedSessionNotice message contexts**: `get_message_html()` and `get_message_plaintext()` accept a `MessageContext` enum `$context` parameter — use `MessageContext::Purchase` for checkout/cart blocking, `MessageContext::Generic` (default) for non-purchase flows.
