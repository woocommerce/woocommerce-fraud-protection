...
- **Separate try-catch blocks are intentional**: In `verify_and_block()`, payment resolution and session verification have independent try-catch blocks so one failing doesn't prevent the other from running. Do not merge them.
...
- **Support email priority**: Use `BlockedSessionNotice::get_support_email()` which follows: reply-to → from → admin email.
