...
src/Internal/FraudProtectionPlugin/Schemas/   Internal DTOs (Address, CartItem, OrderData, ...) + enums (ClearanceStatus)
...
- **Blocked-session message**: The public `BlockedSessionMessage::get_html()` / `get_plaintext()` accept a `MessageContext` enum — use `MessageContext::Purchase` for checkout/cart blocking, `MessageContext::Generic` (default) for non-purchase flows. Internal `BlockedSessionNotice` exposes thin `get_message_html()`/`get_message_plaintext()` delegators for first-party callers; gateway compat layers should depend on the public `BlockedSessionMessage` instead.
