...
- **Block verdicts are per-attempt**: A block verdict applies only to the checkout/payment attempt that produced it — the protector rejects that attempt and nothing is persisted. Every new attempt is re-verified from scratch, so false positives can retry and the `woocommerce_fraud_protection_automated_decision` whitelist filter works as a recovery path for automated blocks (sessions decided by a merchant rule bypass the filter; their recovery path is the merchant editing or deleting the rule). Do NOT reintroduce session-persisted block state, store-wide blocking gates, or cart emptying on block (removed in WOOSUBS-1769; velocity/repeat-abuse handling belongs server-side).
